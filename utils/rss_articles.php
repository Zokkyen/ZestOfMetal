...
function GetArticlesFromTime(SimpleXMLElement $flux, string $time) : array {
    ...
    foreach ($flux->channel->item as $entry) {
        ...
        $weekCurrent = $currentDate->format("W");
        ...
        switch($time) {
            ...
            case 'week' : 
                ...
                break;
            case 'month' : 
                if (strcmp($dateArticleArray[1], $dateCurrentArray[1]) != 0) {
                    $okReading = false;
                }                  
                break;
            case 'archive' : 
                if ($counter == (count($flux->channel->item)-1)) {
                    $okReading = false;
                }
                break;
            default : 
                echo 'Rien de plus à afficher car le paramètre est manquant';
                return $dataToSend[$counter] = '';
        }
        if($okReading) {
